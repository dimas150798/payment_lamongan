...
class C_DataPaket extends CI_Controller
{
    ...
    public function GetDataAjax()
    {
        ...
        foreach ($result as $dataPaket) {
            ...
            $row[] = ++$no;
            $row[] = $dataPaket['name'];
            $row[] = '<div class="text-center">' . 'Rp. ' . number_format($dataPaket['price'], 0, ',', '.') . '</div>';
            $row[] = '<div class="text-center">' . $dataPaket['description'] . '</div>';
            $row[] =
                ...
                <div class="btn-group">
                    ...
                    <div class="dropdown-menu text-black" style="background-color:aqua;">
                        <a onclick="EditPaket(' . $dataPaket['id'] . ')"class="dropdown-item text-black"></i> Edit</a>
                        <a onclick="DeletePaket(' . $dataPaket['id'] . ')" class="dropdown-item text-black"><i class="bi bi-trash3-fill"></i> Hapus</a>
                    </div>
                </div>
                </div>';
            $data[] = $row;
